...
export default function Programcilar() {
  ...
  const [programcilarId, setProgramcilarId] = useState("");
  ...
  const oneCikaninIsmi = () => {
    // Öne çıkan geliştiriciyi bulmak için oneCikanId'yi kullanın.
    const oneCikan = programcilar.find((dev) => dev.id === programcilarId);
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: programcilarId ? "gold" : "royalblue", // 🤔 kutlarken renk gold'a dönecek
  };
